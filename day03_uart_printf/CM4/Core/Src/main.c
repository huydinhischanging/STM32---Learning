...
int main(void)
{
  ...
  MX_UART7_Init();
  /* USER CODE BEGIN 2 */
  setvbuf(stdout, NULL, _IONBF, 0);  // tắt buffer của printf, gửi ngay lập tức từng ký tự
  printf("\r\n--- Day 3: UART7 + printf ---\r\n");
  /* USER CODE END 2 */
...
static void MX_GPIO_Init(void)
{
  ...
  // LED_B (PD11) - dùng để xác nhận vòng lặp M4 còn chạy, tách biệt khỏi vấn đề dây UART
  {
    GPIO_InitTypeDef GPIO_InitStruct = {0};
    __HAL_RCC_GPIOD_CLK_ENABLE();
